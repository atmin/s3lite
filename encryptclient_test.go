...
func TestEncryptedReplicaSurvivesCompaction(t *testing.T) {
	...
	if err := restoreDB(ctx, db.cfg.replica(), replicaURL, before); err != nil {
	...
	if err := restoreDB(ctx, db.cfg.replica(), replicaURL, after); err != nil {
...
func TestEncryptedReplicaKeyHandling(t *testing.T) {
	...
	t.Run("WrongKey", func(t *testing.T) {
		...
		err := restoreDB(ctx, replicaConfig{EncryptionKey: testKey(0xb4)}, replicaURL, dest)
	...
	t.Run("AbsentKey", func(t *testing.T) {
		...
		err := restoreDB(ctx, replicaConfig{}, replicaURL, dest)
	...
	t.Run("RightKeyStillWorks", func(t *testing.T) {
		...
		if err := restoreDB(ctx, replicaConfig{EncryptionKey: key}, replicaURL, dest); err != nil {
...
func TestEncryptedReplicaMixedWindow(t *testing.T) {
	...
	err = restoreDB(ctx, replicaConfig{EncryptionKey: key, RequireEncrypted: true}, replicaURL,
		filepath.Join(t.TempDir(), "strict.sqlite3"))
