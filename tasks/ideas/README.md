# Ideas — candidates below the frontier

Design directions worth capturing but **not** committed to. Unlike
[the frontier](../README.md) one level up — which holds intended-to-implement
units of work that get deleted once they land — an idea here may sit
indefinitely, get refined, or be dropped. **Nothing in this directory should be
implemented without an explicit decision to promote it first**; promotion is a
move up one level (`git mv tasks/ideas/x.md tasks/`).

Each file states the idea, why it's interesting, and — importantly — the reasons
it isn't a task yet (the tradeoff, the constraint, or the missing prerequisite).

- [cooperative-yield.md](cooperative-yield.md) — bounded-fairness writer handoff: a
  waiter signals intent, the holder voluntarily yields the lease at a safe point,
  with min/max hold to prevent both thrashing and starvation. The release-on-idle
  slice was promoted and has landed (`YieldLease`); the marker/ticketing
  remainder — **do not implement yet.**
- [forwarded-writes.md](forwarded-writes.md) — followers forward writes to the
  current leader (rqlite-style) for a multi-writer *feel* with one physical writer.
  **Do not implement yet** — platform-dependent and against the scale-to-zero grain.
- [cli.md](cli.md) — a `sqlite3`-*familiar* (not drop-in) shell over s3lite: role on the
  command line, reads always local, a writer streaming every second, and a follower
  pulling fresh per statement. **Do not implement yet** — needs a cheap replica-tip
  probe and an exported synchronous `Refresh` first; both are library work.
- [restore-progress-callback.md](restore-progress-callback.md) — a
  `Config.OnRestoreProgress` callback for a live restore %/ETA and stall detection.
  **Do not implement yet** — needs a litestream-fork progress hook, and no consumer
  wants a bar over the restore-observability logging yet.
